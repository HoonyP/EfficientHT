...
#define MAX_LEN 200
#define TEXT_SIZE 34685

int occurrence[TEXT_SIZE] = { 0, };
int matching = 0;
...
int main(int argc, char* argv[]) {
	...
	clock_t table_stime = 0, table_etime = 0;
	clock_t total_stime = 0, total_etime = 0;
	int** PATTERN_SET = NULL;
	int** phi = NULL;
	int** phi_inv = NULL;
	int** E = NULL;
	int table_size = -1;
	int* TEXT = NULL;
	int* hash_Arr = NULL;
	struct inv_H * inverse_hash_Arr = NULL;
	int* check_table = NULL;
	...
	for (int PATTERN_COUNT = 200; PATTERN_COUNT <= MAX_COUNT; PATTERN_COUNT += 200) {
		for (int PATTERN_LEN = 20; PATTERN_LEN <= MAX_LEN; PATTERN_LEN += 20) {
			pre = src = total = 0.0;
			// a = 0;
