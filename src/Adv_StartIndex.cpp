...
void preprocessing_table(int** p, unordered_map<int,int>& HT, int PATTERN_COUNT, int PATTERN_LEN, int* Hash_Arr, inv_H* inverse_Hash, int* check, int table_size, int BLOCK_SIZE) {

	int m = PATTERN_LEN;
	int range = m - BLOCK_SIZE + 1;
	int** p_prime = make_p_prime(p, m, PATTERN_COUNT);

	for (int i = 0; i < PATTERN_COUNT; i++) {
		Hash_Arr[i] = q_gram_H(p_prime[i], range - 1, m, BLOCK_SIZE);
	...
	for (int i = 0;i<table_size;i++){
		HT.insert(pair<int,int>(i, check[i]));
	}
...
void Search_H(int* Text, unordered_map<int,int> HT, int** p, int* Hash_Arr, int** phi_inv, int** E, int PATTERN_COUNT, int PATTERN_LEN, inv_H* inv_Hash, int* check, int table_size, int BLOCK_SIZE) {
	...
	while (start_idx < TEXT_SIZE) {
		...
		int index = HT.find(temp)->second;
		while (inv_Hash[index].FP_value == temp) {
			matching++;
			int P_len = PATTERN_LEN;
			if (Check_OP(Text, p[inv_Hash[index].pattern_num], s, P_len, phi_inv[inv_Hash[index].pattern_num], E[inv_Hash[index].pattern_num])) {
				a++;
			}
			index++;
		}
...
int main(int argc, char* argv[]) {
	...
	for (int PATTERN_COUNT = 200; PATTERN_COUNT <= MAX_COUNT; PATTERN_COUNT += 200) {
		for (int PATTERN_LEN = 8; PATTERN_LEN <= MAX_LEN; /*PATTERN_LEN += 20*/) {
			...
			table_stime = clock();
			preprocessing_phi(PATTERN_SET, phi, phi_inv, E, PATTERN_COUNT, PATTERN_LEN); // PatternSet¿¡ ´ëÇØ POS °è»ê
			preprocessing_table(PATTERN_SET, HT, PATTERN_COUNT, PATTERN_LEN, hash_Arr, inverse_hash_Arr, check_table, table_size, BLOCK_SIZE); // FingerPrint Value·Î Á¤·ÄµÈ FPS °è»ê
			table_etime = clock();
			pre += (float)(table_etime - table_stime) / CLOCKS_PER_SEC * 1000;

			table_stime = clock();
			Search_H(TEXT, HT, PATTERN_SET, hash_Arr, phi_inv, E, PATTERN_COUNT, PATTERN_LEN, inverse_hash_Arr, check_table, table_size, BLOCK_SIZE); // Matching ÁøÇà
			table_etime = clock();
